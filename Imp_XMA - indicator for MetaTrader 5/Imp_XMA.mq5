...
//--- в качестве цвета линии индикатора использован Violet цвет
...
input double Close_coef=1.0;
input double Open_coef=1.0;
input double High_coef=1.0;
input double Low_coef=1.0;
input int Shift=0;                       // Сдвиг индикатора по горизонтали в барах
input int PriceShift=0;                  // Cдвиг индикатора по вертикали в пунктах
//+-----------------------------------+
//--- объявление динамического массива, который будет
//--- в дальнейшем использован в качестве индикаторного буфера
double IndBuffer[];
//---
double Sum_coef;
...
//---  
   Sum_coef=Close_coef+Open_coef+High_coef+Low_coef;
...
//--- инициализации переменной для короткого имени индикатора
   ...
   StringConcatenate(shortname,"XMA(",XLength,", ",Smooth1,")");
...
//--- основной цикл расчёта индикатора
   for(bar=first; bar<rates_total && !IsStopped(); bar++)
     {
      price=(Open_coef*open[bar]+High_coef*high[bar]+Low_coef*low[bar]+Close_coef*close[bar])/Sum_coef;
