...
//--- в качестве цвета медвежьего индикатора использован DarkOrange цвет
...
//--- в качестве цвета бычьего индикатора использован CornflowerBlue цвет
...
input int UpLevel=-30; //уровень перекупленности
input int DnLevel=-70; //уровень перепроданности
//+----------------------------------------------+
...
//--- получение хендла индикатора WPRPeriod
   WPR_Handle=iWPR(Symbol(),NULL,WPRPeriod);
   if(WPR_Handle==INVALID_HANDLE)
     {
      Print("Не удалось получить хендл индикатора WPRPeriod");
      return(INIT_FAILED);
     }
