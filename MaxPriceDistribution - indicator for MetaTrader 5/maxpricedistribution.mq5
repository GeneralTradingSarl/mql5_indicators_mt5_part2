...
#property description "Индикатор показывает гистограмму распределения экстемальных максимумов цен за период в барах от текущего"
...
//+----------------------------------------------+
input string  SirName="MaxPriceDistribution";     //Первая часть имени графических объектов
...
input color PrColor=clrLightPink;                 //цвет количества цен
//+----------------------------------------------+
...
//---- имя для окон данных и лэйба для субъокон 
   string short_name="MaxPriceDistribution";
      ...
      for(int kkk=0; kkk<Pr; kkk++)
        {
         ...
         if(close[bar]==P && close[bar+1]<close[bar] && close[bar]>close[bar-1]) Price[kkk]++;
        }
     }
