...
//--- в качестве цвета медвежьего индикатора использован Crimson цвет
#property indicator_color1  clrCrimson
//--- толщина линии индикатора 1 равна 4
#property indicator_width1  4
//--- отображение бычей метки индикатора
#property indicator_label1  "iMFISign Sell"
//+----------------------------------------------+
...
#property indicator_label2 "iMFISign Buy"
//+----------------------------------------------+
//|  объявление констант                         |
//+----------------------------------------------+
...
double BuyBuffer[];
//---
int MFI_Handle,ATR_Handle,min_rates_total;
//+------------------------------------------------------------------+
...
//--- инициализация глобальных переменных 
   min_rates_total=int(MathMax(MFIPeriod+1,ATR_Period))+1;
//--- получение хендла индикатора MFIPeriod
   MFI_Handle=iMFI(Symbol(),NULL,MFIPeriod,VolumeType);
   if(MFI_Handle==INVALID_HANDLE)
     {
      Print("Не удалось получить хендл индикатора MFIPeriod");
      return(INIT_FAILED);
     }
...
//--- копируем вновь появившиеся данные в массивы
   if(CopyBuffer(MFI_Handle,0,MAIN_LINE,to_copy+1,MFI)<=0) return(RESET);
   ...
   for(bar=limit; bar>=0 && !IsStopped(); bar--)
     {
      BuyBuffer[bar]=NULL;
      SellBuffer[bar]=NULL;
      if(MFI[bar] > DnLevel && MFI[bar+1] <= DnLevel) BuyBuffer[bar] = low[bar] - ATR[bar]*3/8;
		    if(MFI[bar] < UpLevel && MFI[bar+1] >= UpLevel) SellBuffer[bar] = high[bar] + ATR[bar]*3/8;         
     }
//---     
   BuySignal("iRSISign",BuyBuffer,rates_total,prev_calculated,close,spread);
   SellSignal("iRSISign",SellBuffer,rates_total,prev_calculated,close,spread);
//---      
   return(rates_total);
  }
//+------------------------------------------------------------------+
